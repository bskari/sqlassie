...
// ----------------------------------------------------------------------------------------
	...
	operator[] (
		unsigned long pos_parameter
	) const
	{
		return array_nodes[pos_parameter>>mask_size][pos_parameter&mask];
	}
...
// ----------------------------------------------------------------------------------------
	...
	void array_kernel_1<T,mem_manager>::
	set_size (
		unsigned long size_parameter
	)
	{
		if (array_size == 0 && size_parameter != 0)
		{
			const unsigned long new_biggest_node = (size_parameter-1)/(mask+1);
			try
			{
