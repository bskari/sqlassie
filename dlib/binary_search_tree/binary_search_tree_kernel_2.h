...
// ----------------------------------------------------------------------------------------
	...
	) const
	{
		if (node_ptr != NIL)
		{
			if (comp(d , node_ptr->d))
			{
				// go left
				return get_count(d,node_ptr->left);				
			}
			else if (comp(node_ptr->d , d))
			{
				// go right
				return get_count(d,node_ptr->right);
			}
			else
			{
				// go left and right to look for more matches
				return   get_count(d,node_ptr->left) 
					   + get_count(d,node_ptr->right) 
					   + 1;
			}
		}
		return 0;
...
// Older GCC versions instantiate templates at the end of the translation
// unit, so we have to wait until here to disable warnings.
#if GCC_VERSION < 40600
	#pragma GCC diagnostic ignored "-Weffc++"
#endif
