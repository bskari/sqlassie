			...
			for (long r = 0; r < lhs.nr(); r+=bs)
			{
				for (long c = 0; c < lhs.nc(); c+=bs)
				{
					...
					for (long i = 0; i < rhs.nc(); i += bs)
					{
						...
						for (long row = lhs_block.top(); row <= lhs_block.bottom(); ++row)
						{
							for (long col = lhs_block.left(); col <= lhs_block.right(); ++col)
							{
								const typename EXP2::type temp = lhs(row,col);
								for (long iter = rhs_block.left(); iter <= rhs_block.right(); ++iter)
								{
									dest(row,iter) += rhs(col,iter)*temp;
								}
							}
						}
					}
