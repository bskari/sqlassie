...
namespace dlib 
{
	...
	public:
		...
		typedef matrix<type,NR,1,mem_manager_type,layout_type> column_localvector_type;

		typedef matrix<std::complex<type>,0,0,mem_manager_type,layout_type> complex_matrix_type;
		typedef matrix<std::complex<type>,NR,1,mem_manager_type,layout_type> complex_column_localvector_type;
		...
		const complex_column_localvector_type get_eigenvalues (
		) const;

		const column_localvector_type& get_real_eigenvalues (
		) const;

		const column_localvector_type& get_imag_eigenvalues (
		) const;
	...
	private:
		...
		column_localvector_type d;		 /* real part */
		column_localvector_type e;		 /* img part */
...
// ----------------------------------------------------------------------------------------
	...
	) 
	{
		...
		else 
		{
			...
			if (A.nr() > DLIB_LAPACK_EIGENVALUE_DECOMP_SIZE_THRESH)
			{
				matrix<type,0,0,mem_manager_type, column_localmajor_layout> temp, vl, vr;
				temp = A;
...
// ----------------------------------------------------------------------------------------
	...
	template <typename matrix_exp_type>
	const typename eigenvalue_decomposition<matrix_exp_type>::complex_column_localvector_type eigenvalue_decomposition<matrix_exp_type>::
	get_eigenvalues (
	) const
	{
...
// ----------------------------------------------------------------------------------------
	...
	template <typename matrix_exp_type>
	const typename eigenvalue_decomposition<matrix_exp_type>::column_localvector_type& eigenvalue_decomposition<matrix_exp_type>::
...
// ----------------------------------------------------------------------------------------
	...
	hqr2 ()
	{
		...
		while (n_local >= low) 
		{
			...
			if (l == n_local) 
			{
				H(n_local,n_local) = H(n_local,n_local) + exshift;
				d(n_local) = H(n_local,n_local);
				e(n_local) = 0.0;
				n_local--;
				iter = 0;
			...
			{
				w = H(n_local,n_local-1) * H(n_local-1,n_local);
				p = (H(n_local-1,n_local-1) - H(n_local,n_local)) / 2.0;
				q = p * p + w;
				z = sqrt(abs(q));
				H(n_local,n_local) = H(n_local,n_local) + exshift;
				H(n_local-1,n_local-1) = H(n_local-1,n_local-1) + exshift;
				x = H(n_local,n_local);
				...
				if (q >= 0) 
				{
					...
					for (int j = n_local-1; j < nn; j++) 
					{
						z = H(n_local-1,j);
						H(n_local-1,j) = q * z + p * H(n_local,j);
						H(n_local,j) = q * H(n_local,j) - p * z;
					}
					...
					{
						z = H(i,n_local-1);
						H(i,n_local-1) = q * z + p * H(i,n_local);
						H(i,n_local) = q * H(i,n_local) - p * z;
					}
					...
					{
						z = V(i,n_local-1);
						V(i,n_local-1) = q * z + p * V(i,n_local);
						V(i,n_local) = q * V(i,n_local) - p * z;
					}
			...
			else 
			{
				...
				{
					exshift += x;
					for (int i = low; i <= n_local; i++) 
					{
						H(i,i) -= x;
					}
					s = abs(H(n_local,n_local-1)) + abs(H(n_local-1,n_local-2));
					x = y = 0.75 * s;
					w = -0.4375 * s * s;
				}
				...
				for (int k = m; k <= n_local-1; k++) 
				{
					int notlast = (k != n_local-1);
					if (k != m) 
					{
						p = H(k,k-1);
						q = H(k+1,k-1);
					...
					if (s != 0) 
					{
						...
						for (int i = 0; i <= min(n_local,k+3); i++) 
						{
							p = x * H(i,k) + y * H(i,k+1);
							if (notlast) 
							{
		...
		for (n_local = nn-1; n_local >= 0; n_local--) 
		{
			p = d(n_local);
			q = e(n_local);
			...
			{
				int l = n_local;
				H(n_local,n_local) = 1.0;
				for (int i = n_local-1; i >= 0; i--) 
				{
					w = H(i,i) - p;
					r = 0.0;
					for (int j = l; j <= n_local; j++) 
					{
						r = r + H(i,j) * H(j,n_local);
					}
					if (e(i) < 0.0) 
					{
						z = w;
					...
					else 
					{
						...
						else 
						{
							...
							if (abs(x) > abs(z)) 
							{
								H(i+1,n_local) = (-r - w * t) / x;
							} 
							else 
							{
								H(i+1,n_local) = (-s - y * t) / z;
							}
						}
			...
			else if (q < 0) 
			{
				...
				if (abs(H(n_local,n_local-1)) > abs(H(n_local-1,n_local))) 
				{
					H(n_local-1,n_local-1) = q / H(n_local,n_local-1);
					H(n_local-1,n_local) = -(H(n_local,n_local) - p) / H(n_local,n_local-1);
				} 
				else 
				{
					cdiv_(0.0,-H(n_local-1,n_local),H(n_local-1,n_local-1)-p,q);
					H(n_local-1,n_local-1) = cdivr;
					H(n_local-1,n_local) = cdivi;
				}
				H(n_local,n_local-1) = 0.0;
				H(n_local,n_local) = 1.0;
				for (int i = n_local-2; i >= 0; i--) 
				{
					type ra,sa_local,vr,vi;
					ra = 0.0;
					sa_local = 0.0;
					for (int j = l; j <= n_local; j++) 
					{
						ra = ra + H(i,j) * H(j,n_local-1);
						sa_local = sa_local + H(i,j) * H(j,n_local);
					}
					w = H(i,i) - p;
					...
					else 
					{
						...
						else 
						{
							...
							cdiv_(x*r-z*ra+q*sa_local,x*s-z*sa_local-q*ra,vr,vi);
							H(i,n_local-1) = cdivr;
							H(i,n_local) = cdivi;
							if (abs(x) > (abs(z) + abs(q))) 
							{
								H(i+1,n_local-1) = (-ra - w * H(i,n_local-1) + q * H(i,n_local)) / x;
								H(i+1,n_local) = (-sa_local - w * H(i,n_local) - q * H(i,n_local-1)) / x;
							}
							else 
							{
								cdiv_(-r-y*H(i,n_local-1),-s-y*H(i,n_local),z,q);
								H(i+1,n_local-1) = cdivr;
								H(i+1,n_local) = cdivi;
							}
						}
						...
						t = max(abs(H(i,n_local-1)),abs(H(i,n_local)));
						if ((eps * t) * t > 1) 
						{
							for (int j = i; j <= n_local; j++) 
							{
								H(j,n_local-1) = H(j,n_local-1) / t;
								H(j,n_local) = H(j,n_local) / t;
							}
						}
					}
